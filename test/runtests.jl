...
@testset "local registry configuration" begin
    ...
    @test_throws ErrorException config(["local_registries" => url1])
    ...
    @test_throws ErrorException config(["local_registries" => Dict(uuid1 => 17)])
...
mktempdir(@__DIR__) do test_dir
    ...
    # So can a package from each of them. The package registered in the
    # second registry is only found after the first storage server has
    # been tried and turned out not to have it.
